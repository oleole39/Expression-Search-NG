...
//var { xnote } = ChromeUtils.import("resource://expressionsearch/modules/xnotestore.jsm");
...
var NUEVO_FULLTEXT_SQL1 =
  ...
  GlodaMsgSearcher.old_buildFulltextQuery = GlodaMsgSearcher.buildFulltextQuery;
   

  GlodaMsgSearcher.buildFulltextQuery = function() {
    let query = Gloda.newQuery(GlodaConstants.NOUN_MESSAGE, {
      noMagic: true,
      explicitSQL: NUEVO_FULLTEXT_SQL1,
...
var ExpressionSearchChrome = {
  ...
  delayedOnSearchKeyPress: async function (event) {
    /*
        let { GlodaMsgSearcher } = ChromeUtils.import(
          "resource://expressionsearch/modules/msgSearch.jsm"
        );
    */
  //  console.log("delayedOnSearchKeyPress");
    let me = ExpressionSearchChrome;
    let win = ExpressionSearchChrome.getWinFromEvent(event);
    me.isEnter = 0;
      ...
      if (typeof (searchValue) != 'undefined' && searchValue != '') {
        ...
        if (event.ctrlKey || event.metaKey) { // create quick search folder
          ...
          if (searchValue != '') {
            ...
            //to show as a list, we need the searchresults as finished collection, so currently await instead of listener
            //glodaList shows all occurances, glodaFacet only one per email
            let coll = new win.GlodaMsgSearcher(null, searchValue).getCollection();
            await new Promise(resolve => setTimeout(resolve, 500));
            tabmail.openTab("glodaList", {
              collection: coll //new win.GlodaMsgSearcher(null, searchValue).getCollection(null, searchValue)
  ...
  getNotes: async function () {
    xnote.notes = await ExpressionSearchCommon.notifyBackground({ command: "getAllXNotes" })
  //  console.log("get notes", xnote.notes);

  },

  onSearchBarFocus: async function (event) {
 //   console.log("searchbar focus");

    xnote.XNotesInstalled = await ExpressionSearchCommon.notifyBackground({ command: "XNotesInstalled" });
    xnote.BookmarksInstalled = await ExpressionSearchCommon.notifyBackground({ command: "BookmarksInstalled" });
 //   console.log("XNotesInstalled", xnote.XNotesInstalled, "BM", xnote.BookmarksInstalled);
    if (xnote.XNotesInstalled) xnote.notes = await ExpressionSearchCommon.notifyBackground({ command: "getAllXNotes" });
    if (xnote.BookmarksInstalled) xnote.bookmarks = await ExpressionSearchCommon.notifyBackground({ command: "getAllBookmarks" });
 //   console.log("searchbar focus", xnote.BookmarksInstalled, xnote.XNotesInstalled, xnote.notes);
  ...
  // select first message, expand first container if closed
  selectNextOrPreviousMessage: function (win, upDown, needSelect) { // needSelect: false:no foucus change, undefined:focus pan, true: focus to pan and select message
    if (!win || !win.document) return;
    let doc = win.document;
    let aNode = doc.getElementById(this.textBoxDomId);
    let gFolderDisplay = win.gFolderDisplay;
      ...
      if (treeView && dbViewWrapper && treeView.rowCount > 0) {
        ...
        if (typeof (needSelect) == 'undefined' || needSelect) {
          ...
          if (needSelect) {
            ...
            //threadPane.view.selection.select(threadPane.currentIndex);
            var row = treeView.isContainer(0) && dbViewWrapper.showGroupedBySort ? 1 : 0;
            row = treeView.selection.currentIndex;//currentSelection;
            //row  = win.messenger.navigatePos;
  //          console.log("row", win.messenger.navigatePos, treeView.selection.currentIndex, treeView.selection, treeView);
            // gFolderDisplay.navigate(Ci.nsMsgNavigationType.forward, true);
            row += upDown;
            if (row < 0) row = 0;
            if (row > treeView.rowCount - 1) row = 0;//treeView.rowCount ;
            treeView.selection.select(row);
            gFolderDisplay.tree.ensureRowIsVisible(row);
          } // needSelect
        } // undefined or needSelect
  ...
  loadInto3pane: function (win) {
    ...
    try {
      ...
      /*
            let  getMsg = win.document.getElementById("button-getmsg");
            let  getMsgNew = getMsg.cloneNode(true);
            getMsgNew.label = "&getAllNewMsgCmd.label;";
            let getAll = getMsgNew.querySelector("#button-getAllNewMsg");
            let getAllNew = getAll.cloneNode();
      
            getMsgNew.id= getAllNew.id;
            getMsgNew.label = getAllNew.label;
            getMsgNew.tooltiptext=  "22";
  //          console.log("get", getMsg, getMsgNew);
            getAll.remove();    //  getMsg.
            getMsg.parentNode.replaceChild(getMsgNew, getMsg);
            //getAll.parentNode.replaceChild(getMsgNew, getAll);
  ...
  Load: function (win) {
    ...
    if (type == 'mail:3pane') {
      ...
      //  ExpressionSearchChrome.load_qfbPivot(win);
